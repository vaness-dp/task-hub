// components/project-stats/ProjectStatCard.tsx
import cn from 'clsx'
...
export function ProjectStatCard({ projectStat }: Props) {
	return (
		...
		>
			{/* Градиентный оверлей */}
			<div className="absolute inset-0 bg-gradient-to-br from-black/10 via-transparent to-black/20 dark:from-black/20 dark:via-transparent dark:to-black/40 rounded-3xl"></div>
			...
			<div className="flex items-center justify-between relative z-10">
				<div className="flex flex-col">
					<span className="text-3xl font-bold mb-2 text-white dark:text-[#f5f5f5] drop-shadow-md group-hover:scale-110 transition-transform duration-300 origin-left">
						{projectStat.id === 3 ? formatMinutes(projectStat.value) : projectStat.value}
					</span>
					<span className="text-sm text-white/90 dark:text-neutral-300 drop-shadow-sm font-medium group-hover:text-white dark:group-hover:text-[#f5f5f5] transition-colors duration-300">
						{projectStat.label}
					</span>
				</div>
			...
			<div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/15 dark:via-white/8 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 transform -skew-x-12 -translate-x-full group-hover:translate-x-full"></div>
		</div>
	)
}
